...
async function initializeTest() {
    ...
    // FULL REFERENCE TEXT
    window.state.paragraph = fullParagraph;

    // words for comparison engine
    window.state.defaultWords = fullParagraph.split(" ");

    // reset paragraph index system
    window.state.currentParagraphIndex = 0;

    // remove chunk system completely
   window.paragraphs = [fullParagraph];
}
// -------------------- DOM ready setup --------------------
...
if (timerSelect) {
    timerSelect.addEventListener("change", function () {
        window.state.totalTime = parseInt(this.value);
        refreshTest(); // restart test with new timer
    });
}
...
function refreshTest() {
    ...
    window.state.totalTime = window.state.totalTime || 600;
    window.state.isTestRunning = false;
...
setTimeout(() => {
    ...
    for (let i = 0; i < validChunks.length; i++) {
        ...
        errorTablesHTML += `
            <table class="error-table" width="100%"border="2px" style="border-collapse:collapse;">
                <thead>
                    <tr>
                        <th width="50%">#Original Paragraph</th>
                        <th width="50%">#Typed Paragraph</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
    ...
    if ($("gross-speed-value")) $("gross-speed-value").textContent = `${grossSpeed} WPM`;
